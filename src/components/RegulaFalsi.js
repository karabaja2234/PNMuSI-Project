...
export default function RegulaFalsi(props) {
    ...
    //States used for handling the graph of the RegulaFalsi Algorithm's results
    const ref = useRef();
    const [data, setData] = useState({})

    //Method that uses the RegulaFalsi Algorithm and handles it's result
    const calculateRegulaFalsi = () => {
        try {
            const regulaFalsiAlgorithm = new RegulaFalsiAlgorithm(functionDefinition, startInterval, endInterval, precision, maxIterations);
            const response = regulaFalsiAlgorithm.calculateRegulaFalsi();
            if(response) {
                setResults(response)
